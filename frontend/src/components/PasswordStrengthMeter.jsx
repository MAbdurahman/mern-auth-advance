      ...
      <div className='mt-2 space-y-1 tracking-wider'>
         {criteria.map((item) => (
            <div key={item.label} className='flex items-center text-xs'>
               {item.met ? (
                  <Check className='size-4 text-augmented-600 mr-2' />
               ) : (
                  <X className='size-4 text-augmented-600 mr-2' />
               )}
               <span className={item.met ? "text-augmented-600" : "text-neutral-400"}>{item.label}</span>
            </div>
         ))}
      </div>
   );
   ...
   return (
      <div className='mt-2'>
         <div className='flex justify-between items-center mb-1'>
            <span className='text-xs text-blue-01-700 font-semibold tracking-wider'>Password Strength</span>
